...
async function createStartScript(serverDir, config) {
  ...
  const maxMemoryMB = memoryMB * 1024;
  const minMemoryMB = Math.floor(maxMemoryMB * 0.5);
...
JAVA_OPTS="-Xms${minMemoryMB}M -Xmx${maxMemoryMB}M ..."
...
async function downloadServer() {
  try {
    ...
    await writeProgress(serverId, 90, 'setup', 'Created start.sh');
